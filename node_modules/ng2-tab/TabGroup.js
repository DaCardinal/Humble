"use strict";
var core_1 = require("@angular/core");
var Tab_1 = require("./Tab");
var TabGroup = (function () {
    function TabGroup() {
        this.closeOthers = false;
        this.showArrows = false;
        this.expandAll = false;
        this.etabs_tabgroup = "etabs-tabgroup";
    }
    TabGroup.prototype.ngAfterContentInit = function () {
        var _this = this;
        if (this.expandAll) {
            this.closeOthers = false;
            this.oldGroups = this.groups.toArray();
            this.oldGroups.forEach(function (group) {
                group.openOnInitialization();
                
            });
            // we subscribe for changes, and if new groups are added we open them automatically
            this.subscription = this.groups.changes.subscribe(function (change) {
                var newGroups = _this.groups.toArray().filter(function (group) {
                    return _this.oldGroups.indexOf(group) === -1;
                });
                newGroups.forEach(function (group) {
                    group.openOnInitialization();
                });
                _this.oldGroups = _this.groups.toArray();
            });
        }
    };
    TabGroup.prototype.ngOnDestroy = function () {
        if (this.subscription)
            this.subscription.unsubscribe();
    };
    TabGroup.prototype.closeAll = function () {
        this.groups.toArray().forEach(function (group) {
            group.isOpened = false;
        });
    };
    TabGroup.decorators = [
        { type: core_1.Component, args: [{
                    selector: "etabs-tabgroup",
                    template: "\n<div class=\"etabs-tabs\"><ng-content></ng-content>\n<div #component></div></div>\n",
                    host: {'[class]': 'etabs_tabgroup'}
                },] },
    ];
    /** @nocollapse */
    TabGroup.ctorParameters = function () { return []; };
    TabGroup.propDecorators = {
        'closeOthers': [{ type: core_1.Input },],
        'showArrows': [{ type: core_1.Input },],
        'expandAll': [{ type: core_1.Input },],
        'groups': [{ type: core_1.ContentChildren, args: [core_1.forwardRef(function () { return Tab_1.Tab; }),] },],
        'component': [{ type: core_1.ViewChild, args: ['component',] },],
    };
    return TabGroup;
}());
exports.TabGroup = TabGroup;
//# sourceMappingURL=TabGroup.js.map